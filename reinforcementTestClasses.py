        ...
        for state in states:
            ...
            for action in actions:
                ...
                if action in possibleActions:
                    qValues[action][state] = agent.computeQValue(state, action)
                else:
...
class QLearningTest(testClasses.TestCase):
    ...
    def runAgent(self, moduleDict, numExperiences):
        ...
        policy = {}
        for state in states:
            values[state] = agent.computeActionAndValueFromQValues(state)[1]
            policy[state] = agent.computeActionAndValueFromQValues(state)[0]
            possibleActions = self.grid.getPossibleActions(state)
            for action in actions:
                if action not in qValues:
                    qValues[action] = {}
    ...
    def testEpsilonGreedy(self, moduleDict, tolerance=0.025):
        ...
        for state in self.grid.getStates():
            ...
            if numLegalActions <= 1:
                continue
            numGreedyChoices = 0
            optimalAction = agent.computeActionAndValueFromQValues(state)[0]
            for iteration in range(self.numIterations):
                # assume that their computeActionFromQValues implementation is correct (q2 tests this)
